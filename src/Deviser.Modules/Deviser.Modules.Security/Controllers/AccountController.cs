...
using Deviser.Modules.Security.Extensions;

namespace Deviser.Modules.Security.Controllers
{
    [Module(ModuleGlobals.ModuleName)]
    [Authorize]
    public class AccountController : DeviserController
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.RequiresTwoFactor)
                {
                    ...
                    return RedirectToAction(nameof(LoginWith2fa), "Account", new { area = "Security" });
                    //return RedirectToAction(nameof(LoginWith2fa), ControllerContext.RouteData);
                    //return RedirectToAction(nameof(LoginWith2fa), "Account", new { modules = ModuleGlobals.ModuleName, returnUrl, model.RememberMe });
                }
                if (result.IsLockedOut)
                {
                    _logger.LogWarning("User account locked out.");
                    return RedirectToAction(nameof(Lockout), new { module = ModuleGlobals.ModuleName });
                }
                else
                {
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            if (remoteError != null)
            {
                ...
                return RedirectToAction(nameof(Login), new { module = ModuleGlobals.ModuleName });
            }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null || !(await _userManager.IsEmailConfirmedAsync(user)))
                {
                    ...
                    return RedirectToAction(nameof(ForgotPasswordConfirmation), new { module = ModuleGlobals.ModuleName });
                }

                // For more information on how to enable account confirmation and password reset please visit http://go.microsoft.com/fwlink/?LinkID=532713
                // Send an email with this link
                var code = await _userManager.GeneratePasswordResetTokenAsync(user);                
                var callbackUrl = Url.RouteUrl(Globals.moduleRoute, new { Controller = "Account", Action = "ResetPassword", userId = user.Id, code = code }, protocol: HttpContext.Request.Scheme);
                //Url.Action("ResetPassword", "Account", new { userId = user.Id, code = code }, protocol: HttpContext.Request.Scheme);
        ...
        private IActionResult RedirectToLocal(string returnUrl = null)
        {
            ...
            if (IsAjaxRequest)
            {
                ...
                result.StatusCode = StatusCodes.Status302Found;
                return Ok(result);
            }
            else
            {
